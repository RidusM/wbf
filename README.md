...
Что внутри?
    ...
    * zlog — логирование на базе zerolog (по умолчанию в формате JSON) | **Deprecated**
    * logger - гибкая обертка для популярных логеров с управляемым контекстом и ротацией логов
...
#### Логирование (zlog | Deprecated)
```go
zlog.Init()
zlog.Logger.Info().Msg("Hello")
```

#### Логирование (logger | Recomended)
```go 
engine := logger.ZapEngine
